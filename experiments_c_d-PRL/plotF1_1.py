...
def plot(r):
    ...
    algos = ["Euclidean", "GMML", "ITML", "LMNN", "IMLS", "IML"]
    colors = ["grey", "tomato", "tan", "dodgerblue", "orange", "#229944"]
    patterns = ['', '/', '', '\\', '', '//']
    bar_width = 0.75 / len(algos)

    matplotlib.rcParams['font.size'] = 14
    for da in ["Mean results over the 22 datasets"]:
        ...
        for i, a in enumerate(algos):
            ...
            ax.bar(index+i*bar_width, vals, bar_width,
                   ...
                   label=a, zorder=2, hatch=patterns[i])
        ax.legend(ncol=len(algos), loc=9, bbox_to_anchor=(0.5, 1.32))
        plt.savefig("res_" + da + "_full.pdf", bbox_inches="tight")
        fig.clf()
        plt.close(fig)
